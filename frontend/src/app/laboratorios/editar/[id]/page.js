...
export default function EditarLaboratorioPage() {
  ...
  const handleSubmit = async e => {
    ...
    setLoading(true);
    try {
      const res = await updateLaboratorio(id, form);
      if (res && res.CodLab) {
        if (typeof window !== 'undefined') {
          localStorage.setItem('laboratorioMensaje', 'Laboratorio editado exitosamente');
        }
        router.push('/laboratorios');
      } else {
        setError("Error al editar el laboratorio");
      }
    } catch {
      setError("Error al editar el laboratorio");
    }
  ...
  return (
    <div className="w-full max-w-5xl mx-auto ml-4 mr-8">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-semibold text-gray-800">Editar Laboratorio</h1>
      </div>
      <div className="pt-8">
